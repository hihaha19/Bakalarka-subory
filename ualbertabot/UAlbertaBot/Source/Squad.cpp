...
void Squad::addUnitsToMicroManagers()
{
    ...
    for (auto & unit : m_units)
    {
        ...
        {
            if (unit->getType() == BWAPI::UnitTypes::Protoss_Corsair) {//unit->gettype == 60
...
bool Squad::needsToRegroup()
{
    ...
    if (!unitClosest || unitClosest->getType() == BWAPI::UnitTypes::Protoss_Corsair)
    {
    ...
    if (unitClosest->getType() == BWAPI::UnitTypes::Protoss_Corsair) retreat = 0;
    int switchTime = 100;
    bool waiting = false;
    ...
    if (retreat && unitClosest->getType() != BWAPI::UnitTypes::Protoss_Corsair)
    {
