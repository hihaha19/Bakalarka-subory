...
int RangedManager::getAttackPriority(BWAPI::Unit rangedUnit, BWAPI::Unit target)
{
    ...
    // ak náš target má zbraò proti lietajúcim jednotkám, isThreat == True alebo ak nelieta a nepriate¾ má pozemnú zbraò
...
//    if (target->getType() == BWAPI::UnitTypes::Special_Power_Generator)
    ...
    else if (targetType == BWAPI::UnitTypes::Zerg_Hatchery)
    {
        return 400;
    }
    // next is special buildings
...
{
     const BWAPI::Unitset & rangedUnits = getUnits();
    //const BWAPI::UnitTypes::Protoss_Corsair & rangedUnits = getUnits()
    // 
    // figure out targets
    BWAPI::Unitset rangedUnitTargets;
    std::copy_if(targets.begin(), targets.end(), std::inserter(rangedUnitTargets, rangedUnitTargets.end()), [](BWAPI::Unit u) { return u->isVisible(); });
